#!/usr/bin/env python
"""blog_figures.py — render the LiftOn v1.0.10 announcement's composite figures.
...
# blog-facing tool labels: the devel build is announced as "LiftOn v1.0.10"
...
def fig_accuracy(fw, vc):
    ...
    _panel_title(axB, "B",
                 f"LiftOn v1.0.10 beats the best single method on {_nwin}/{len(pts)} datasets")
...
def fig_completeness(fw):
    ...
    axA.margins(y=0.16)
    _panel_title(axA, "A", "v1.0.8 crashes partway; v1.0.10 finishes")
    ...
    if rows:
        ...
        axB.grid(axis="x", alpha=0.3)
        axB.margins(x=0.12)
    _panel_title(axB, "B", "v1.0.10 lifts gene-like types v1.0.8 drops")
...
def fig_validity(fw):
    ...
    # mark the one dataset where v1.0.10 has MORE errors than v1.0.8 (honest)
...
def fig_full_accuracy(fw):
    ...
    _panel_title(axA, "A", "LiftOn v1.0.10 leads the single-method baselines on every whole genome")
...
def fig_full_validity(fw):
    ...
    ax.grid(axis="x", alpha=0.3)
    ax.margins(x=0.16)
    ax.set_title("Cleaner output on every whole genome — LiftOn v1.0.10 vs v1.0.8",
                 fontsize=12, fontweight="bold", loc="left")
